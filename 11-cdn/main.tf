resource "aws_cloudfront_distribution" "daws78s" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.daws78s.domain_name
        zone_id = aws_cloudfront_distribution.daws78s.hosted_zone_id
      }
    }
  ]
}
